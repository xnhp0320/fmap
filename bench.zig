...
pub fn benchmark(comptime B: type, allocator: Allocator) !void {
    ...
    inline for (functions) |def| {
        inline for (args, 0..) |arg, index| {
            var runtimes = try allocator.alloc(u64, max_iterations);
            ...
            {
                const ctx = benchCtx{ .timer = &timer, .runtime = &runtimes[i], .allocator = allocator };

                const res = switch (@TypeOf(arg)) {
                    void => @field(B, def.name)(ctx),
                    else => @field(B, def.name)(arg, ctx),
                };
...
test "benchmark" {
    ...
    try benchmark(struct {
        ...
        pub fn sum_slice(slice: []const u8, ctx: benchCtx) u64 {
            ctx.timer.reset();
            var res: u64 = 0;
            for (slice) |item|
                res += item;
        ...
        pub fn sum_reader(slice: []const u8, ctx: benchCtx) u64 {
            ctx.timer.reset();
